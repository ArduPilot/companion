...
sudo apt install libssl-dev libusb-1.0-0-dev pkg-config -y
# This is for ccmake
sudo apt install build-essential cmake cmake-curses-gui -y

# Graphics libraries - for SDK's OpenGL-enabled examples
sudo apt install libgtk-3-dev libglfw3-dev libgl1-mesa-dev libglu1-mesa-dev -y
